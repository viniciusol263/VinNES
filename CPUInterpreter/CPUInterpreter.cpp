        ...
        case 0xE000:
        {
            switch(opcode & 0xFF) {
                ...
                {   
                    ...
                    if(keyEvent.key.keysym.sym == Utils::keyButtons[m_CpuRegisters.V[(opcode >> 8) & 0x0F]]) {
                        if(keyEvent.key.state == SDL_PRESSED) {
                            m_CpuRegisters.PC += 2;
                        }
                    }
                    m_CpuRegisters.PC += 2;
                ...
                {   
                    ...
                    SDL_PollEvent(&keyEvent);
                    if(keyEvent.key.keysym.sym == Utils::keyButtons[m_CpuRegisters.V[(opcode >> 8) & 0x0F]]) {
                        if(keyEvent.key.state == SDL_RELEASED) {
                            m_CpuRegisters.PC += 2;
                        }
                    }
                    m_CpuRegisters.PC += 2;
